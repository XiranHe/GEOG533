...
std.err <- 0.2 / q95
sqrt.n <- sd / std.err
n <- round(sqrt.n * sqrt.n,digits = 0)
n+1
```
...
```{r}
if(result$p.value>0.05) {
  print("the town has a mobility rate that is same with the national average")
} else {
...
result <- t.test(x1,x2,var.equal = F,conf.level = 0.95)
if(result$p.value>0.05) {
  print("the mean length of unemployment same as the residents of two local communities")
} else {
...
result.95$conf.int[1:2]
result.95$p.value
if(result.95$p.value>0.10) {
...
result.90$conf.int[1:2]
result.90$p.value
if(result.90$p.value>0.10) {
  print("mobility rates in the two communities is not different")
} else {
  print("mobility rates in the two communities is different")
}
...
result.a <- t.test(x1,x2,var.equal = F,conf.level = 0.95)
if(result.a$p.value>0.05) {
